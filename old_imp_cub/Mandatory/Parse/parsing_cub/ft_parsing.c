...
/*   Updated: 2025/03/12 18:17:54 by bikourar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// int util_color2(char *s, int *count_nbr1, int *i, int *r)
// {
// 	if (s[(*i)] == '+')
// 		(*i)++;
...
int ft_util1(t_buff *mem)
{
	...
	while (mem->array[i])
	{
		...
		if (c != '\0' &&  c != '1' && c != 'N'  && c != 'S' && c != 'W' && c != 'E' && c != 'C' && c != 'F')
...
int util_walls(char **array, int count, int i)
{
	...
	&& array[count][i] != 'W' && array[count][i] != 'E')
...
int cheek_map_char(char **array, int count)
{
	...
	while (array[count])
	{
		...
		while (array[count][j])
		{
			...
			&& array[count][j] != 'S' && array[count][j] != 'N' && array[count][j] != 'W' && array[count][j] != 'E')
				return (ft_error(NULL, NULL, "Error\nMap chararcters\n"), 1);
			j++;
		}
		count++;
